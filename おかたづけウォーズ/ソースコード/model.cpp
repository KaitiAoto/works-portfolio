...
CModel* CModel::Create(const char* pFilename, D3DXVECTOR3 pos, D3DXVECTOR3 rot)
{
	...
	//初期化
	if (FAILED(pModel->Init(pFilename, pos, rot)))
	{
		return nullptr;
	}
...
void CModel::Draw(void)
{
	...
	for (int nCntMat = 0; nCntMat < (int)m_dwNumMat; nCntMat++)
	{
		...
		else if(m_bColorChange == false)
		{
			...
			{
				pMat[nCntMat].MatD3D.Diffuse.a = forcedMat.Diffuse.a;
			}
			else
			{
...
D3DXVECTOR3 CModel::GetSize(void)
{
	...
	D3DXVECTOR3 vtxMin = VECTOR3_NULL;	// モデルの最小値
	D3DXVECTOR3 vtxMax = VECTOR3_NULL;	// モデルの最大値
	if (this != nullptr)
	{
		if (m_pMesh == nullptr)
		{
	...
	while (1)
	{
		...
		if (strcmp(aString, "SCRIPT") == 0)
		{
			while (1)
			{
				...
				{
					fscanf(pFile, "%s %d", equal, &nNumModel);
				...
				else if (strcmp(aString, "MODEL_FILENAME") == 0)
				{
					...
					{
						fscanf(pFile, "%s %s", equal, &aString[0]);
				...
				else if (strcmp(aString, "CHARACTERSET") == 0)
				{
					while (1)
					{
						...
						{
							fscanf(pFile, "%s %d", equal, &nNum_Parts);
						}
						else if (strcmp(aString, "PARTSSET") == 0)
						{
							while (1)
							{
								...
								{
									fscanf(pFile, "%s %d", equal, &nParent);
									if (Index < NUM_MODEL && nParent < NUM_MODEL &&
										model[Index] != nullptr && model[nParent] != nullptr)
									{
